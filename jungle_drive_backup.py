...
class Animal:
    ...
    def update(self):
        self.animation_frame += 0.04  # Even slower, more realistic animation
        # Very subtle movement for realistic feel
        if self.type in ["squirrel", "monkey"]:
            # Small animals move occasionally
            if random.randint(0, 600) == 0:  # Very infrequent movement
                self.direction *= -1
            self.x += self.direction * self.speed * 0.3  # Very slow movement
        elif self.type == "bird":
            # Birds fly more naturally
            if random.randint(0, 500) == 0:
                self.direction *= -1
            self.x += self.direction * self.speed * 0.2
            # Very subtle vertical movement for birds
            self.y += math.sin(self.animation_frame * 1.5) * 0.3
    ...
    def draw_squirrel(self, screen, screen_x):
        ...
        # Tail (big and fluffy)
        tail_curve = math.sin(self.animation_frame * 2) * 5
        pygame.draw.ellipse(screen, (139, 69, 19), (screen_x - 25, self.y - 25 + tail_curve, 20, 35))
        pygame.draw.ellipse(screen, RETRO_BLACK, (screen_x - 25, self.y - 25 + tail_curve, 20, 35), 3)
        # Eyes
    ...
    def draw_monkey(self, screen, screen_x):
        ...
        # Arms (animated)
        arm_offset = math.sin(self.animation_frame) * 4
        pygame.draw.line(screen, RETRO_BROWN, (screen_x - 12, self.y - 15), (screen_x - 20, self.y - 10 + arm_offset), 6)
        pygame.draw.line(screen, RETRO_BROWN, (screen_x + 12, self.y - 15), (screen_x + 20, self.y - 10 - arm_offset), 6)
        # Legs
    ...
    def draw_bird(self, screen, screen_x):
        # More realistic bird without red trail (updated)
        # Body (brown/natural color)
        ...
        # Wing (subtle animation, natural colors)
        wing_flap = math.sin(self.animation_frame * 3) * 4  # Reduced flap intensity
        pygame.draw.ellipse(screen, (101, 67, 33), (screen_x - 10, self.y - 15 + wing_flap, 22, 10))
        pygame.draw.ellipse(screen, RETRO_BLACK, (screen_x - 10, self.y - 15 + wing_flap, 22, 10), 2)
        # Head (natural brown)
...
class Menu:
    ...
    def draw(self, screen):
        ...
        RetroFont.render_retro_text(screen, "RETRO MILITARY ADVENTURE", SCREEN_WIDTH//2 - 240, title_y + 25, 32, RETRO_WHITE, True)
...
class Game:
    ...
    def draw_terrain(self):
        ...
        # Realistic jungle background elements (static, not rotating)
        for i in range(int(self.camera_x // 120), int((self.camera_x + SCREEN_WIDTH) // 120) + 1):
            x = i * 120
            if 0 <= x < self.world_width:
                screen_x = x - self.camera_x
                if (x // 120) % 3 == 0:  # Every third position
                    # Background trees (static)
                    tree_y = self.terrain_height[x] - random.randint(60, 120)
                    tree_size = random.randint(25, 45)
                    # Tree trunk
                ...
                elif (x // 120) % 4 == 1:  # Different spacing for variety
                    # Background bushes
                    bush_y = self.terrain_height[x] - random.randint(20, 40)
                    bush_size = random.randint(15, 30)
    ...
    def draw_hud(self):
        ...
        pygame.draw.rect(screen, RETRO_YELLOW, pause_bg, 2)
    ...
    def handle_input(self):
        ...
        # Detect keys just pressed (not held)
        for key in range(len(keys_pressed)):
            if keys_pressed[key] and key not in self.keys_pressed_last_frame:
                keys_just_pressed.add(key)
        
        self.keys_pressed_last_frame = {key for key in range(len(keys_pressed)) if keys_pressed[key]}
        
        if self.state == MENU:
            menu_result = self.menu.handle_input(keys_just_pressed)
            if menu_result == 0:  # Start the Drive
                self.state = PLAYING
                self.init_game_world()
            elif menu_result == 1:  # Rest on Tyres (Resume if paused)
                if hasattr(self, 'jeep') and self.jeep and not self.game_over and not self.game_won:
                    self.state = PLAYING
            elif menu_result == 2:  # Park Out of Jungle (Quit)
                return False
            ...
            if pygame.K_p in keys_just_pressed or pygame.K_ESCAPE in keys_just_pressed or pygame.K_RETURN in keys_just_pressed:
                self.state = PLAYING
        ...
        while running:
            ...
            if self.state == MENU:
                ...
                if self.state == PAUSED:
                    # Pause overlay with better styling
                    overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT))
                    ...
                    # Pause panel
                    pause_panel = pygame.Rect(SCREEN_WIDTH//2 - 200, SCREEN_HEIGHT//2 - 100, 400, 200)
                    pygame.draw.rect(self.screen, RETRO_BLACK, pause_panel)
                    pygame.draw.rect(self.screen, RETRO_YELLOW, pause_panel, 4)
                    
                    RetroFont.render_retro_text(self.screen, "GAME PAUSED", SCREEN_WIDTH//2 - 120, SCREEN_HEIGHT//2 - 70, 48, RETRO_YELLOW)
                    RetroFont.render_retro_text(self.screen, "Press P to Resume", SCREEN_WIDTH//2 - 100, SCREEN_HEIGHT//2 - 20, 32, RETRO_WHITE)
                    RetroFont.render_retro_text(self.screen, "Press ESC for Menu", SCREEN_WIDTH//2 - 110, SCREEN_HEIGHT//2 + 20, 32, RETRO_WHITE)
...
if __name__ == "__main__":
    ...
    game.run()
    def draw_elephant(self, screen, screen_x):
        # More realistic retro elephant
        # Body (larger and more detailed)
        pygame.draw.ellipse(screen, (140, 140, 140), (screen_x - 40, self.y - 30, 80, 60))
        pygame.draw.ellipse(screen, RETRO_BLACK, (screen_x - 40, self.y - 30, 80, 60), 4)
        # Head (bigger)
        pygame.draw.circle(screen, (140, 140, 140), (int(screen_x - 50), int(self.y - 20)), 28)
        pygame.draw.circle(screen, RETRO_BLACK, (int(screen_x - 50), int(self.y - 20)), 28, 4)
        # Trunk (more realistic curve)
        trunk_points = [(screen_x - 70, self.y - 15), (screen_x - 85, self.y - 5), (screen_x - 80, self.y + 20)]
        pygame.draw.lines(screen, (140, 140, 140), False, trunk_points, 12)
        pygame.draw.lines(screen, RETRO_BLACK, False, trunk_points, 3)
        # Legs (chunky retro style)
        for leg_x in [screen_x - 30, screen_x - 10, screen_x + 10, screen_x + 30]:
            pygame.draw.rect(screen, (120, 120, 120), (leg_x - 6, self.y + 25, 12, 25))
            pygame.draw.rect(screen, RETRO_BLACK, (leg_x - 6, self.y + 25, 12, 25), 3)
        # Ears (large)
        pygame.draw.ellipse(screen, (120, 120, 120), (screen_x - 75, self.y - 35, 25, 30))
        pygame.draw.ellipse(screen, RETRO_BLACK, (screen_x - 75, self.y - 35, 25, 30), 3)
        # Eye
        pygame.draw.circle(screen, RETRO_BLACK, (int(screen_x - 55), int(self.y - 25)), 4)
        pygame.draw.circle(screen, RETRO_WHITE, (int(screen_x - 53), int(self.y - 27)), 2)
        # Tusks
        pygame.draw.line(screen, RETRO_WHITE, (screen_x - 65, self.y - 10), (screen_x - 70, self.y + 5), 4)
        pygame.draw.line(screen, RETRO_WHITE, (screen_x - 60, self.y - 10), (screen_x - 65, self.y + 5), 4)
    
    def draw_tiger(self, screen, screen_x):
        # More realistic retro tiger
        # Body (bigger and more detailed)
        pygame.draw.ellipse(screen, (255, 140, 0), (screen_x - 35, self.y - 22, 70, 45))
        pygame.draw.ellipse(screen, RETRO_BLACK, (screen_x - 35, self.y - 22, 70, 45), 4)
        # Head (bigger)
        pygame.draw.circle(screen, (255, 140, 0), (int(screen_x + 30), int(self.y - 15)), 22)
        pygame.draw.circle(screen, RETRO_BLACK, (int(screen_x + 30), int(self.y - 15)), 22, 4)
        # Stripes (more realistic)
        for stripe_x in [screen_x - 25, screen_x - 10, screen_x + 5, screen_x + 20]:
            pygame.draw.line(screen, RETRO_BLACK, (stripe_x, self.y - 20), (stripe_x, self.y + 15), 5)
        # Head stripes
        for i in range(3):
            stripe_angle = -30 + i * 30
            start_x = screen_x + 25 + 15 * math.cos(math.radians(stripe_angle))
            start_y = self.y - 20 + 15 * math.sin(math.radians(stripe_angle))
            end_x = screen_x + 25 + 8 * math.cos(math.radians(stripe_angle))
            end_y = self.y - 20 + 8 * math.sin(math.radians(stripe_angle))
            pygame.draw.line(screen, RETRO_BLACK, (start_x, start_y), (end_x, end_y), 3)
        # Legs (chunky)
        for leg_x in [screen_x - 25, screen_x - 8, screen_x + 8, screen_x + 25]:
            pygame.draw.rect(screen, (255, 140, 0), (leg_x - 4, self.y + 18, 8, 20))
            pygame.draw.rect(screen, RETRO_BLACK, (leg_x - 4, self.y + 18, 8, 20), 2)
        # Ears (triangular)
        pygame.draw.polygon(screen, (255, 140, 0), [(screen_x + 20, self.y - 30), (screen_x + 25, self.y - 35), (screen_x + 30, self.y - 30)])
        pygame.draw.polygon(screen, (255, 140, 0), [(screen_x + 30, self.y - 30), (screen_x + 35, self.y - 35), (screen_x + 40, self.y - 30)])
        pygame.draw.polygon(screen, RETRO_BLACK, [(screen_x + 20, self.y - 30), (screen_x + 25, self.y - 35), (screen_x + 30, self.y - 30)], 2)
        pygame.draw.polygon(screen, RETRO_BLACK, [(screen_x + 30, self.y - 30), (screen_x + 35, self.y - 35), (screen_x + 40, self.y - 30)], 2)
        # Eyes
        pygame.draw.circle(screen, RETRO_BLACK, (int(screen_x + 25), int(self.y - 18)), 4)
        pygame.draw.circle(screen, RETRO_BLACK, (int(screen_x + 35), int(self.y - 18)), 4)
        pygame.draw.circle(screen, RETRO_WHITE, (int(screen_x + 26), int(self.y - 19)), 2)
        pygame.draw.circle(screen, RETRO_WHITE, (int(screen_x + 36), int(self.y - 19)), 2)
        # Tail
        pygame.draw.line(screen, (255, 140, 0), (screen_x - 35, self.y), (screen_x - 50, self.y - 10), 6)
        pygame.draw.line(screen, RETRO_BLACK, (screen_x - 35, self.y), (screen_x - 50, self.y - 10), 2)
    ...
    def draw_deer(self, screen, screen_x):
        # More realistic retro deer (updated)
        # Body
        pygame.draw.ellipse(screen, RETRO_BROWN, (screen_x - 25, self.y - 25, 50, 40))
        pygame.draw.ellipse(screen, RETRO_BLACK, (screen_x - 25, self.y - 25, 50, 40), 4)
        # Head
        pygame.draw.circle(screen, (160, 100, 50), (int(screen_x + 20), int(self.y - 25)), 15)
        pygame.draw.circle(screen, RETRO_BLACK, (int(screen_x + 20), int(self.y - 25)), 15, 3)
        # Antlers (more detailed)
        pygame.draw.line(screen, (101, 67, 33), (screen_x + 15, self.y - 35), (screen_x + 10, self.y - 45), 4)
        pygame.draw.line(screen, (101, 67, 33), (screen_x + 25, self.y - 35), (screen_x + 30, self.y - 45), 4)
        pygame.draw.line(screen, (101, 67, 33), (screen_x + 12, self.y - 42), (screen_x + 8, self.y - 48), 3)
        pygame.draw.line(screen, (101, 67, 33), (screen_x + 28, self.y - 42), (screen_x + 32, self.y - 48), 3)
        # Legs (retro style)
        for leg_x in [screen_x - 15, screen_x - 5, screen_x + 5, screen_x + 15]:
            pygame.draw.rect(screen, RETRO_BROWN, (leg_x - 3, self.y + 10, 6, 25))
            pygame.draw.rect(screen, RETRO_BLACK, (leg_x - 3, self.y + 10, 6, 25), 2)
        # Eye
        pygame.draw.circle(screen, RETRO_BLACK, (int(screen_x + 25), int(self.y - 28)), 3)
        pygame.draw.circle(screen, RETRO_WHITE, (int(screen_x + 26), int(self.y - 29)), 1)
        # Tail (retro white)
        pygame.draw.circle(screen, RETRO_WHITE, (int(screen_x - 25), int(self.y - 10)), 6)
        pygame.draw.circle(screen, RETRO_BLACK, (int(screen_x - 25), int(self.y - 10)), 6, 2)
